...
def calculate_adx(df, period=14):
    """Calcula el indicador ADX (Average Directional Index) para medir fuerza de tendencia."""
    alpha = 1 / period
    ...
    di_sum = df['plus_dm'].ewm(alpha=alpha, adjust=False).mean() + df['minus_dm'].ewm(alpha=alpha, adjust=False).mean()
    df['plus_di'] = np.where(di_sum == 0, 0, 100 * (df['plus_dm'].ewm(alpha=alpha, adjust=False).mean() / df['atr_adx']))
    df['minus_di'] = np.where(di_sum == 0, 0, 100 * (df['minus_dm'].ewm(alpha=alpha, adjust=False).mean() / df['atr_adx']))
    
    dx_den = df['plus_di'] + df['minus_di']
    df['dx'] = np.where(dx_den == 0, 0, 100 * abs(df['plus_di'] - df['minus_di']) / dx_den)
...
def analyze_market(symbol="BTC/USDT"):
    try:
        ...
        # Cálculo de ATR para Stop Loss y Take Profit
        ...
        # Filtro de fuerza de tendencia ADX (< 20 indica mercado lateral / sin fuerza)
        adx_actual = calculate_adx(df_15m)
        mercado_lateral = adx_actual < 20

        trend_filter = "NEUTRAL"
        accion = "⏳ MERCADO LATERAL / RANGO PLANO (ADX < 20)"
        estrategia = f"• **ADX:** {adx_actual:.1f} (Sin fuerza tendencial)\n• **Soporte:** ${soporte:,.2f} | **Resistencia:** ${resistencia:,.2f}\n• *Bot en pausa defensiva por rango lateral.*"
        ...
        # Si el mercado NO es lateral (ADX >= 20), evaluamos las señales con autoridad
        if not mercado_lateral:
            if ema9 > ema21 and es_alcista_1h and precio_actual > resistencia * 0.995 and tiene_volumen:
                ...
                accion = "🟢 COMPRA / LONG (¡Tendencia y Ruptura Validadas!)"
                estrategia = f"• **ADX:** {adx_actual:.1f} (Tendencia fuerte)\n• **Entrada (Long):** ${precio_actual:,.2f}\n• **Stop Loss (SL):** ${sl_long:,.2f}\n• **Take Profit (TP):** ${tp_long:,.2f}"
        ...
        mensaje = (
            ...
            f"📊 **Fuerza Tendencia (ADX):** {adx_actual:.1f} | **RSI:** {rsi:.1f}\n"
    ...
    try:
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if adx >= 20 and vol_act > (vol_prom * 1.1):
                    if ema9 > ema21:
                        oportunidades.append(f"🟢 **{symbol}**: Alcista con ADX {adx:.1f} (${precio:,.2f})")
                    elif ema9 < ema21:
                        oportunidades.append(f"🔴 **{symbol}**: Bajista con ADX {adx:.1f} (${precio:,.2f})")
            except Exception:
                continue
        ...
        return "📡 **¡ALERTA DE OPORTUNIDAD INSTITUCIONAL (Filtro ADX)**:\n\n" + "\n".join(oportunidades)
